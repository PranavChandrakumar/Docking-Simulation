...
if (Target_rA == Target_rP):
    Target_omega = 0
if (Target_T >= np.sqrt((4*np.pi*Target_alpha**3)/Planet_mu)):
    Target_T = np.mod(Target_T,np.sqrt((4*np.pi*Target_alpha**3)/Planet_mu))
...
if (Spacecraft_T >= np.sqrt((4*np.pi*Spacecraft_alpha**3)/Planet_mu)):
    Spacecraft_T = np.mod(Spacecraft_T,np.sqrt((4*np.pi*Spacecraft_alpha**3)/Planet_mu))
...
def CalculateDeltaV(R, v_initial, target_parameter):
...
Planet = sphere(pos = vector(0,0,0), radius = Planet_Radius, color = color.blue)
